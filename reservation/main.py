...
if not DEBUG:
    ...
    log(f"waiting for next day, sleep {NEED_TO_WAIT}+2 secs")
    time.sleep(NEED_TO_WAIT+2)
...
for info in [(TIME1, FIELD1), (TIME2, FIELD2)]:
    ...
    if raw_data.status_code == 200 and str(html_data) == "ok":
        log(f"reserved {data} {Fore.GREEN+Style.BRIGHT}successfully{Fore.RESET+Style.RESET_ALL},"
